...
/**
 * Plantilla HTML para recordatorio de cita al solicitante.
 */
function plantillaRecordatorioSolicitante({ nombre, pastorNombre, fecha, hora, motivo }) {
  const fechaFormato = new Date(fecha).toLocaleDateString('es-CO', {
    weekday: 'long',
    year: 'numeric',
    month: 'long',
    ...
    <body>
      <div class="container">
        ...
        <div class="body">
          <p style="margin:0 0 8px"><span class="badge">Recordatorio de cita</span></p>
          <p style="font-size:15px; color:#334155; margin:0 0 16px">
            Hola <strong>${nombre}</strong>, tienes una cita pastoral programada para mañana:
          </p>
          <div class="detail">
            <div class="detail-row">
              <span class="detail-label">Pastor</span>
              <span class="detail-value">${pastorNombre}</span>
            </div>
            <div class="detail-row">
              <span class="detail-label">Fecha</span>
              <span class="detail-value">${fechaFormato}</span>
...
module.exports = { enviarCorreo, plantillaRecordatorio, plantillaRecordatorioSolicitante, plantillaNuevaVisita, formatTime12h };
